...
def pmos_vth(cv, fins=1, fingers=1, start='A') :
	lib = cv.lib()
	...
	vth_ovlp_act_x = int(0.030 * dbu)
	vth_ovlp_act_y = int(0.064 * dbu)
	...
	# Create vth
	vth_lyr = tech.getLayerNum("VTH", "drawing")
	vth_rect = Rect(-length/2-vth_ovlp_act_x, -width/2-vth_ovlp_act_y, length/2+vth_ovlp_act_x, width/2+vth_ovlp_act_y)
	vth = cv.dbCreateRect(vth_rect, vth_lyr);
