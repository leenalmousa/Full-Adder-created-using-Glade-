...
def pmos_thk(cv, fins=1, fingers=1, start='A') :
	lib = cv.lib()
	...
	pim_ovlp_act_x = int(0.030 * dbu)
	pim_ovlp_act_y = int(0.040 * dbu)
	nw_ovlp_act_x = int(0.032 * dbu)
	nw_ovlp_act_y = int(0.056 * dbu)
	thk_ovlp_act_x = int(0.030 * dbu)
	thk_ovlp_act_y = int(0.064 * dbu)
	...
	# Create PIM
	pim_lyr = tech.getLayerNum("PIM", "drawing")
	pim_rect = Rect(-length/2-pim_ovlp_act_x, -width/2-pim_ovlp_act_y, length/2+pim_ovlp_act_x, width/2+pim_ovlp_act_y)
	pim = cv.dbCreateRect(pim_rect, pim_lyr);

	# Create thkox
	thk_lyr = tech.getLayerNum("THKOX", "drawing")
	thk_rect = Rect(-length/2-thk_ovlp_act_x, -width/2-thk_ovlp_act_y, length/2+thk_ovlp_act_x, width/2+thk_ovlp_act_y)
	thk = cv.dbCreateRect(thk_rect, thk_lyr);
	
	# Create NW
	nw_lyr = tech.getLayerNum("NW", "drawing")
	nw_rect = Rect(-length/2-nw_ovlp_act_x, -width/2-nw_ovlp_act_y, length/2+nw_ovlp_act_x, width/2+nw_ovlp_act_y)
	nw_shape = cv.dbCreateRect(nw_rect, nw_lyr);
	net = cv.dbCreateNet("B")
	pin = cv.dbCreatePin("S", net, DB_PIN_INOUT)
	cv.dbCreatePort(pin, nw_shape)
